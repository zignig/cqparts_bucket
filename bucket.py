...
class Bucket(cqparts.Part):
    ...
    def make(self):
        ...
        # inner bucket
        inner  = cq.Workplane("XY").workplane(offset=self.thickness)\
            .circle(self.diambot-self.thickness)\
            .workplane(offset=self.height)\
            .circle(self.diamtop-self.thickness)\
            .loft(filled=True,combine=True)
        upperRim = cq.Workplane("XY").workplane(offset=self.height-self.rimHeight).circle(self.diamtop+self.thickness).extrude(self.rimHeight)
