...
class _Case(cqparts.Part):
    ...
    def make(self):
        ...
        # cutout
        col = wp.box(self.length-self.thickness
                     ,self.width-self.ofs
                     ,self.height
                     ,centered=(True,True,False))
        # cutout
        coh = wp.box(self.length-self.ofs
    ...
    def screw_points(self):
        wp = cq.Workplane("XY")
        h = wp.rect(self.length-self.ofs/2
                    ,self.width-self.ofs/2
                    ,forConstruction=True).vertices()
        return h.objects

    def mate_bottom(self,explode=0):
        return Mate(self, CoordSystem(
            origin=(0, 0, -2*explode),
            xDir=(1, 0, 0),
            normal=(0, 0,-1)
        ))
...
class _CaseLid(_Case):
    ...
    def make(self):
        ...
        # add the slab on top
        b = wp.transformed(offset=(0,0,self.height))\
...
class _MountedCase(cqparts.Assembly):
    ...
    def make_constraints(self):
        ...
        for i,j in enumerate(self.sp):
            ...
            mates.append(
                Coincident(
                    self.components[self.item_name(i)].mate_origin,
                    m
                )
            )
        return mates
