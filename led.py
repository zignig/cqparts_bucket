...
class LED(cqparts.Part):
    ...
    def make(self):
        ...
        base = cq.Workplane("XY").circle((self.base_rim + self.diam) / 2).extrude(-self.base_thickness)
        mark = cq.Workplane("XY").rect(self.diam+self.base_rim,self.base_thickness).extrude(-self.base_thickness)
