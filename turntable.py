...
class Disc(box.Top):
    ...
    def mate_top(self):
        return Mate(
            self, CoordSystem(origin=(0, 0,self.thickness), xDir=(1, 0, 0), normal=(0, 0, 1))
        )
...
class DiscDrive(cqparts.Assembly):
    ...
    def make_constraints(self):
        ...
        const.append(
            Coincident(boss.mate_top(),disc.mate_origin)
        )
        const.append(
            Coincident(motor.mate_tip(),boss.mate_origin)
        )
        const.append(
            Fixed(disc.mate_top())
        )
        if self.mount is not None:
            const.append(
                Coincident(self.mount.mate_origin,motor.mate_origin)
            )
        return const
...
class TurnTable(box.Boxen):
    ...
    def make_components(self):
        ...
        mid = Mid(width=self.width, length=self.length-2*self.thickness)
        comps["mid"] = mid
        comps["top"].diameter = self.width*0.9
        comps['drive'] = self.drive(mount=mid,diameter=self.width*0.9-self.clearance)
        return comps
    ...
    def make_constraints(self):
        ...
        const.append(
            Coincident(drive.mate_origin,top.mate_top())
        )
        return const
